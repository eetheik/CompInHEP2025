...
/**********************************************************************/
// double SM_HIGGS_MASS
static void setPara(double SM_HIGGS_MASS) {
...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    std::cerr << "Usage: " << argv[0] << " <SM_HIGGS_MASS>\n";
    return 1;
  }

  double SM_HIGGS_MASS = atof(argv[1]);

  setFlags();
  setPara(SM_HIGGS_MASS);  
  higgsCorr();  // FHHiggsCorr has to be run before FHCouplings

  // This is the Fortran synopsis translated into C
  int error;
  ...
  double SM_HIGGS_WIDTH = GammaSMTot(3);
  std::cout << SM_HIGGS_MASS << "\t" << SM_HIGGS_WIDTH << std::endl;
